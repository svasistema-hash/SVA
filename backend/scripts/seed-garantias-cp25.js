...
//
// Doble modo:
//   - CLI: `npm run seed:garantias-cp25` (corre y cierra DB al final).
//   - Módulo: `require('./scripts/seed-garantias-cp25').run(dbHandle)` —
//     usado por server.js cuando SEED_GARANTIAS_NOW=true (CP4-B hotfix).
//     En modo módulo NO cierra la DB (el server la sigue usando).
//
...
// ─────────────────────────────────────────────────────────────────
// Función principal — recibe el handle de DB (no la cierra).
// ─────────────────────────────────────────────────────────────────
function run(db) {
  ...
  if (!inst) {
    throw new Error(`institución '${INSTITUCION_SLUG}' no existe. Correr "npm run seed" antes.`);
  }
  ...
  return {
    cliente_masculino_id: cliMascId,
    cliente_femenino_id: cliFemId,
    modelo_id: modId,
    garantia_id: garId,
    contrato_id: ctoId,
  };
...
// ─────────────────────────────────────────────────────────────────
// Modo CLI: `node scripts/seed-garantias-cp25.js`
// (mantiene el comportamiento previo — abre la DB, corre, la cierra).
// ─────────────────────────────────────────────────────────────────
if (require.main === module) {
  ...
  try {
    const result = run(db);
    console.log('\nProbar compilación con: node scripts/compile-and-print.js ' + result.contrato_id);
    db.close();
    process.exit(0);
  } catch (e) {
    console.error('[seed:garantias-cp25] ERROR:', e.message);
    db.close();
    process.exit(1);
  }
}
